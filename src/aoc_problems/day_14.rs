...
impl Nanofactory {
    ...
    fn _create(&mut self, chemical_name: String) -> Result<()> {
        ...
        for input_material in &chemical_rqmts.inputs {
            while self.material_counts[&input_material.chemical] < input_material.amount {
                self._create(chemical_name.clone())?;
            }
        }

        let chemical_count = self.material_counts.entry(chemical_name).or_insert(0);
        *chemical_count += chemical_rqmts.output.amount;

        Ok(())
    }

    fn produce_one_fuel(&mut self) -> Result<()> {
        self._create("FUEL".to_string())?;
